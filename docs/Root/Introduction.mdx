...
1. **[WoolyAI Server](https://hub.docker.com/r/woolyai/server/tags)**: A container that runs on your GPU hosts. It measures and allocates GPU cores and VRAM at runtime across multiple simultaneous pod requests. Includes deterministic scheduling options and deduplicates VRAM across multiple clients to save on expensive GPU memory.

2. **[WoolyAI GPU Operator](https://github.com/Wooly-AI/woolyai-gpu-operator)**: Deployed on Kubernetes, allowing NVIDIA pod deployment with WoolyAI libraries pre-included and configured.
