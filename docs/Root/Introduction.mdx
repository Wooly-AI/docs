...
There is a third component that is available should you not want to use Kubernetes scheduling:

3. **[WoolyAI Controller](./controller/setup)**: A web interface and router that is responsible for routing WoolyAI client(ML container) execution requests to the GPU nodes running WoolyAI Server based on real-time GPU utilization.
- If you're using the Controller and not GPU Operator, you will need to set up the Server from the [Server Setup Guide](/server/setup).
